...
import Mojito from '../build/MojitoTokenMock.json'
...
export async function governanceFixture(
  ...
  // deploy mojito, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const mojito = await deployContract(wallet, Mojito)
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, mojito.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { mojito, timelock, governorAlpha }
}
