...
describe('GovernorAlpha', () => {
  ...
  beforeEach(async () => {
    ...
    mojito = fixture.mojito
    timelock = fixture.timelock
    governorAlpha = fixture.governorAlpha
  })

  it('mojito', async () => {
    const balance = await mojito.balanceOf(wallet.address)
    const totalSupply = await mojito.totalSupply()
  ...
  it('governor', async () => {
    ...
    expect(votingPeriod).to.be.eq(201600)
    const timelockAddress = await governorAlpha.timelock()
    expect(timelockAddress).to.be.eq(timelock.address)
    const mojitoFromGovernor = await governorAlpha.mojito()
    expect(mojitoFromGovernor).to.be.eq(mojito.address)
  })
})
